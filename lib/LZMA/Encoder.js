...
define(['./Base','../RangeCoder','../LZ'], function(Base, RangeCoder, LZ) {
'use strict';
...
function Encoder() {
        ...
        this.LiteralEncoder = function() {
                this.Encoder2 = function() {
                        var encoders = RangeCoder.Encoder.initBitModels(null, 0x300);

                        this.init = function() {
                                RangeCoder.Encoder.initBitModels(encoders);
                        };
                        ...
                        this.getPrice = function(matchMode, matchByte, symbol) {
                                ...
                                if (matchMode) {
                                        for (; i >= 0; i--) {
                                                ...
                                                bit = (symbol >>> i) & 1;
                                                price += RangeCoder.Encoder.getPrice(encoders[((1 + matchBit) << 8) + context], bit);
                                                context = (context << 1) | bit;
                                                if (matchBit !== bit) {
                                                        i--;
                                                        break;
                                ...
                                for (; i >= 0; i--) {
                                        bit = (symbol >>> i) & 1;
                                        price += RangeCoder.Encoder.getPrice(encoders[context], bit);
                                        context = (context << 1) | bit;
                                }
                                return price;
                        };
                ...
                var coders = [];
                var _numPrevBits = -1, _numPosBits = -1, _posMask;
                ...
                this.getSubCoder = function(pos, prevByte) {
                        return coders[((pos & _posMask) << _numPrevBits) + (prevByte >> (8 - _numPrevBits))];
                };
        };

        this.LenEncoder = function() {
                var choice = RangeCoder.Encoder.initBitModels(null, 2);
                var lowCoder = [], midCoder = [];
                var highCoder = new RangeCoder.BitTreeEncoder(kNumHighLenBits);
                var posState;

                for (posState = 0; posState < kNumPosStatesEncodingMax; posState++) {
                        lowCoder[posState] = new RangeCoder.BitTreeEncoder(kNumLowLenBits);
                        midCoder[posState] = new RangeCoder.BitTreeEncoder(kNumMidLenBits);
                }

                this.init = function(numPosStates) {
                        RangeCoder.Encoder.initBitModels(choice);
                ...
                this.setPrices = function(posState, numSymbols, prices, st) {
                        var a0 = RangeCoder.Encoder.getPrice0(choice[0]);
                        var a1 = RangeCoder.Encoder.getPrice1(choice[0]);
                        var b0 = a1 + RangeCoder.Encoder.getPrice0(choice[1]);
                        var b1 = a1 + RangeCoder.Encoder.getPrice1(choice[1]);
                        var i;
        ...
        this.LenPriceTableEncoder = function() {
                ...
                this.encode = function(rangeEncoder, symbol, posState) {
                       this.prototype.encode.call(this, rangeEncoder, symbol, posState);
        ...
        var _isMatch = RangeCoder.Encoder.initBitModels(null, kNumStates << kNumPosStatesBitsMax);
        var _isRep = RangeCoder.Encoder.initBitModels(null, kNumStates);
        var _isRepG0 = RangeCoder.Encoder.initBitModels(null, kNumStates);
        var _isRepG1 = RangeCoder.Encoder.initBitModels(null, kNumStates);
        var _isRepG2 = RangeCoder.Encoder.initBitModels(null, kNumStates);
        var _isRep0Long = RangeCoder.Encoder.initBitModels(null, kNumStates << kNumPosStatesBitsMax);
        var _posSlotEncoder = [];
        var _posEncoders = RangeCoder.Encoder.initBitModels(null, kNumFullDistances-kEndPosModelIndex);
        var _posAlignEncoder = new RangeCoder.BitTreeEncoder(kNumAlignBits);
        ...
        this.init = function() {
                ...
                for (i = 0; i < kNumLenToPosStates; i++) {
                        _posSlotEncoder[i] = new RangeCoder.BitTreeEncoder(kNumPosSlotBits);
                }
        ...
        this.getRepLen1Price = function(state, posState) {
                return RangeCoder.Encoder.getPrice0(_isRepG0[state.index]) +
                        RangeCoder.Encoder.getPrice0(_isRep0Long[(state.index << kNumPosStatesBitsMax) + posState]);
        };

        this.getPureRepPrice = function(repIndex, state, posState) {
                var price;
                if (repIndex === 0) {
                        price = RangeCoder.Encoder.getPrice0(_isRepG0[state.index]);
                        price += RangeCoder.Encoder.getPrice1(_isRep0Long[(state.index << kNumPosStatesBitsMax) + posState]);
                } else {
                        price = RangeCoder.Encoder.getPrice1(_isRepG0[state.index]);
                        if (repIndex === 1) {
                                price += RangeCoder.Encoder.getPrice0(_isRepG1[state.index]);
                        } else {
                                price += RangeCoder.Encoder.getPrice1(_isRepG1[state.index]);
                                price += RangeCoder.Encoder.getPrice(_isRepG2[state.index], repIndex - 2);
                        }
                }
                return price;
        };
        ...
        this.backward = function(cur) {
            var posMem, backMem, posPrev, backCur, backRes;
        ...
        this.getOptimum = function(position) {
                ...
                _optimum[1].price = RangeCoder.Encoder.getPrice0(_isMatch[(_state.index << kNmPosStatesBitsMax) + posState]) +
                        _literalEncoder.getSubCoder(position, _previousByte).getPrice(!_state.isCharState(), matchByte, currentByte);
                _optimum[1].makeAsChar();

                matchPrice = RangeCoder.Encoder.getPrice1(_isMatch[(_state.index << kNumPosStatesBitsMax) + posState]);
                repMatchPrice = matchPrice + RangeCoder.Encoder.getPrice1(_isRep[_state.index]);
                ...
                normalMatchPrice = matchPrice + RangeCoder.Encoder.getPrice0(_isRep[_state.index]);
                ...
                for(;;) {
                        ...
                        curAnd1Price = curPrice +
                                RangeCoder.Encoder.getPrice0(_isMatch[(state.index << kNumPosStatesBitsMax) + posState]) +
                        ...
                        matchPrice = curPrice + RangeCoder.Encoder.getPrice1(_isMatch[(state.index << kNumPosStatesBitsMax) + posState]);
                        repMatchPrice = matchPrice + RangeCoder.Encoder.getPrice1(_isRep[state.index]);
                        ...
                        if (!nextIsChar && matchByte !== currentByte) {
                                ...
                                if (lenTest2 >= 2) {
                                        ...
                                        nextRepMatchPrice = curAnd1Price +
                                                RangeCoder.Encoder.getPrice1(_isMatch[(state2.index << kNumPosStatesBitsMax) + posStateNext]) +
                                                RangeCoder.Encoder.getPrice1(_isRep[state2.index]);
                                ...
                                if (lenTest < numAvailableBytesFull) {
                                        ...
                                        if (lenTest2 >= 2) {
                                                ...
                                                curAndLenCharPrice = repMatchPrice +
                                                        ...
                                                        RangeCoder.Encoder.getPrice0(_isMatch[(state2.index << kNumPosStatesBitsMax) + posStateNext]) +
                                                ...
                                                nextMatchPrice = curAndLenCharPrice + RangeCoder.Encoder.getPrice1(_isMatch[(state2.index << kNumPosStatesBitsMax) + posStateNext]);
                                                nextRepMatchPrice = nextMatchPrice + RangeCoder.Encoder.getPrice1(_isRep[state2.index]);
                        ...
                        if (newLen >= startLen) {
                                normalMatchPrice = matchPrice + RangeCoder.Encoder.getPrice0(_isRep[state.index]);
                                        ...
                                        if (lenTest === _matchDistances[offs]) {
                                                if (lenTest < numAvailableBytesFull) {
                                                        ...
                                                        if (lenTest2 >= 2) {
                                                                state2 = state.clone();
                                                                state2.updateMatch();
                                                                posStateNext = (position + lenTest) & _posStateMax;
                                                                curAndLenCharPrice = curAndLenPrice +
                                                                        RangeCoder.Encoder.getPrice0(_isMatch[(state2.index << kNumPosStatesBitsMax) + posStateNext]) +
                                                                        _literalEncoder.getSubCoder(position + lenTest, matchFinder.getIndexByte(lenTest - 2))
        ...
        this.writeEndMarker = function(posState) {
                ...
                _rangeEncoder.encode(_isMatch, (_state.index << kNumPosStatesBitsMax) + posState, 1);
                _rangeEncoder.encode(_isRep, _state.index, 0);
                _state.updateMatch();
